"""
PPO with RNN (GRU) for LunarLander-v2

Proximal Policy Optimization with recurrent neural network for temporal reasoning.
Uses PSCN feature extraction with MLPRNN (3:1 MLP to GRU ratio) for efficient
temporal processing. Implements dual-clip PPO for stable learning.
...
- PSCN (Parallel Split Concatenate Network) feature extraction
- MLPRNN hybrid architecture
...
class Config:
    def __init__(self):
        ...
        self.save_path = "./checkpoints/PPO_RNN_LunarLander.pth"
...
class PPORNNTrainer:
    def __init__(self, config: Config):
        ...
        self.net = ActorCritic(state_dim, action_dim, hidden_size=64).to(config.device)
