...
class Config:
    def __init__(self):
        ...
        self.mhc_dim = 256  # mHC feature dimension (Increased for better capacity)
        ...
        self.seq_len = 8  # Sequence length for RNN processing
        self.batch_size = 128  # Batch size (number of sequences)
        ...
        self.entropy_coef = 0.015  # Entropy bonus coefficient
        self.erc_beta_low = 0.06  # ERC lower bound
        self.erc_beta_high = 0.06  # ERC upper bound
...
class URNN(nn.Module):
    def __init__(
        self, input_size: int, hidden_size: int, layer: Type[nn.Module], *args, **kwargs
    ):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, hidden_state: Optional[torch.Tensor]):
        ...
        if hidden_state is None:
            hidden_state = torch.zeros(
                1, batch_size, self.hidden_size * self.chunk_size, device=x.device
            )
        else:
...
class RND(nn.Module):
    def __init__(self, input_dim, embed_dim):
        ...
        self.predictor, self.target = [
            PSCN(input_dim=input_dim, output_dim=embed_dim, depth=depth)
            for _ in range(2)
        ]
...
class PPOTrainer:
    ...
    def collect_experience(self):
        ...
        for _ in range(self.cfg.update_freq):
            ...
            with torch.no_grad():
                action, log_prob, value, next_hidden_state, predict, target, entropy = (
                    self.model.get_action(state_tensor, hidden_state)
                )
    ...
    def update_model(self, advantages, returns):
        ...
        actions = torch.tensor(self.buffer.actions, dtype=torch.long).to(
            self.cfg.device
        )
        ...
        old_values = torch.tensor(self.buffer.values, dtype=torch.float32).to(
            self.cfg.device
        )
        hidden_states = torch.tensor(
            np.array(self.buffer.hidden_states), dtype=torch.float32
        ).to(self.cfg.device)
        ...
        for _ in range(self.cfg.num_epochs):
            ...
            for start in range(0, self.num_sequences, self.cfg.batch_size):
                ...
                # ERC computation [cite: 122, 131, 132]
                ...
                covs = (new_log_probs - new_log_probs.mean()) * (
                    adv_batch_flat - adv_batch_flat.mean()
                )
                ...
                value_clip = old_values_flat + (values_flat - old_values_flat).clamp(
                    -self.cfg.clip_eps_min, self.cfg.clip_eps_max
                )
                value_loss1 = (values_flat - ret_batch_flat).pow(2)
                value_loss2 = (value_clip - ret_batch_flat).pow(2)
                value_loss = 0.5 * self.masked_mean(
                    torch.max(value_loss1, value_loss2), corr
                )
...
if __name__ == "__main__":
    ...
    print("\nTraining completed or interrupted, starting final test...")
    ppo.test()
