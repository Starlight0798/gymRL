"""
DDPG (Deep Deterministic Policy Gradient) for Pendulum-v1

An actor-critic algorithm for continuous action spaces that uses deterministic policy
gradient with experience replay and soft target network updates.
...
class DDPGTrainer:
    def __init__(self, config: Config):
        ...
        self.critic_target = Critic(state_dim, action_dim, config.hidden_dim).to(
            config.device
        )
    ...
    def select_action(
        ...
        if not deterministic:
            noise = np.random.normal(
                0, self.cfg.noise_std * self.action_bound, size=action.shape
            )
